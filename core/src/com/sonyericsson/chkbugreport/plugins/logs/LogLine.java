...
public class LogLine extends DocNode {
  ...
  private static final Comparator<Decorator> sDecorSorter =
      new Comparator<Decorator>() {
        @Override
  ...
  public void setHidden(boolean b) { mHidden = b; }

  public boolean isHidden() { return mHidden; }

  public void addStyle(String style) { css += " " + style; }
  ...
  private void prepareChildren(Renderer r) { super.prepare(r); }
  ...
  public LogLineProxy symlink() { return new LogLineProxy(this); }
  ...
  private boolean parseFmtStd(BugReportModule br, String line) {
    ...
    if (tagEnd < 20)
      return false; // this is weird... abort
    int p0 = tagEnd - 7;
    int p1 = tagEnd - 1;
    char c = line.charAt(p0);
    if (c != '(')
      return false; // not this format
    c = line.charAt(p1);
    if (c != ')')
      return false; // not this format
    if (line.charAt(20) != '/')
      return false; // strange log format
    c = line.charAt(0);
    if (c < '0' && c > '9')
      return false; // strange log format
  ...
  /**
   * Parse a log line from logs generated by brat scripts (adb logcat -v
   * threadtime)
   */
  ...
  /**
   * Parse a log line in the short format (no timestamp, default when just
   * running "adb logcat")
   */
   ...
   * If a given field does not exists, null will be returned (instead of
  ...
  protected void renderThis(Renderer r) throws IOException {
    if (mDecors == null) {
      r.println("<div class=\"log-line " + css + "\" id=\"l" + id + "\">" +
                HtmlUtil.escape(line) + "</div>");
    } else {
      // So, this is tricky, since we have to render piecewise.
      // There can be any number of decorator segments, and they can even
      // overlap. Since we are rendering html, we ignore segments which
      // intersect others, i.e. we accept only those overlapping ones, which are
      // fully contained in the previous one
      ...
      while (nextEnd >= 0 || nextDecor != null) {
        ...
        if ((nextEnd >= 0 && nextDecor != null &&
             nextEnd <= nextDecor.getStart()) ||
            (nextEnd >= 0 && nextDecor == null)) {
        ...
        if (nextEnd >= 0 && nextDecor != null &&
            nextEnd > nextDecor.getStart() && nextEnd < nextDecor.getEnd()) {
          // Ignore next decor
  ...
  /* package */ static class LogLineProxy extends DocNode {
    ...
    public LogLineProxy(LogLine logLine) { mLogLine = logLine; }
